...
export async function validateOrderFinancials(
  ...
  try {
    const { subtotal, deliveryFee, total, productosBase, AstroBarCommission, couponDiscount } = req.body;
    ...
    const platformCommission =
      typeof AstroBarCommission === "number" && AstroBarCommission > 0
        ? AstroBarCommission
...
export async function calculateCommissions(
  ...
  try {
    const { total, deliveryFee = 0, productosBase, AstroBarCommission } = req.body;
