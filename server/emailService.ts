...
const FROM_EMAIL = "AstroBar <noreply@AstroBar.app>";
...
export async function sendVerificationEmail(
  ...
  try {
    const { data, error } = await resend!.emails.send({
      from: FROM_EMAIL,
      to: email,
      subject: "Verifica tu cuenta de AstroBar",
      html: `
        <!DOCTYPE html>
        <html>
        <head>
        ...
        <body style="font-family: 'Nunito', Arial, sans-serif; background-color: #FAFAFA; margin: 0; padding: 20px;">
          <div style="max-width: 600px; margin: 0 auto; background-color: #FFFFFF; border-radius: 16px; padding: 40px; box-shadow: 0 2px 8px rgba(0,0,0,0.08);">
            <div style="text-align: center; margin-bottom: 32px;">
              <h1 style="color: #00C853; font-size: 32px; margin: 0;">AstroBar</h1>
              <p style="color: #757575; margin-top: 8px;">Tu Promociones Nocturnas de confianza</p>
            </div>
            
            <h2 style="color: #212121; font-size: 24px; margin-bottom: 16px;">Hola ${name},</h2>
            
            <p style="color: #424242; font-size: 16px; line-height: 1.6;">
              Gracias por registrarte en AstroBar. Para completar tu registro y empezar a pedir, 
            ...
            <p style="color: #9E9E9E; font-size: 12px; text-align: center;">
              AstroBar - Promociones Nocturnas en Autlán, Argentina<br>
...
export async function sendOrderConfirmationEmail(
  ...
  try {
    const { data, error } = await resend!.emails.send({
      from: FROM_EMAIL,
      to: email,
      subject: `Pedido confirmado #${orderId.slice(-6)} - AstroBar`,
      html: `
        <!DOCTYPE html>
        <html>
        <head>
        ...
        <body style="font-family: 'Nunito', Arial, sans-serif; background-color: #FAFAFA; margin: 0; padding: 20px;">
          <div style="max-width: 600px; margin: 0 auto; background-color: #FFFFFF; border-radius: 16px; padding: 40px; box-shadow: 0 2px 8px rgba(0,0,0,0.08);">
            ...
            <p style="color: #757575; font-size: 14px;">
              Puedes seguir el estado de tu pedido en la app de AstroBar.
            </p>
            
            <hr style="border: none; border-top: 1px solid #E0E0E0; margin: 32px 0;">
            
            <p style="color: #9E9E9E; font-size: 12px; text-align: center;">
              AstroBar - Promociones Nocturnas en Autlán, Argentina
            </p>
          </div>
        </body>
        </html>
