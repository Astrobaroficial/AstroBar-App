      ...
      <View style={tabStyles.sectionTabs}>
        {(["all", "open", "closed"] as const).map((f) => (
          <Pressable
            key={f}
            onPress={() => setFilter(f)}
            style={[tabStyles.sectionTab, filter === f && { backgroundColor: AstroBarColors.primary }]}
          >
          ...
          <View key={ticket.id} style={[tabStyles.card, { backgroundColor: theme.card }]}>
            <View style={tabStyles.cardHeader}>
              ...
              <View style={[tabStyles.priorityBadge, {
                backgroundColor:
                  ticket.priority === "high" ? AstroBarColors.error + "20" :
                  ticket.priority === "medium" ? AstroBarColors.warning + "20" :
                  AstroBarColors.success + "20"
              }]}>
                <ThemedText type="small" style={{
                  color:
                    ticket.priority === "high" ? AstroBarColors.error :
                    ticket.priority === "medium" ? AstroBarColors.warning :
            ...
            <View style={tabStyles.cardActions}>
              <Pressable
                ...
                style={[tabStyles.actionBtn, { backgroundColor: AstroBarColors.primary + "20" }]}
              >
              ...
              {ticket.status !== "closed" ? (
                <>
                  <Pressable
                    ...
                    style={[tabStyles.actionBtn, { backgroundColor: AstroBarColors.warning + "20" }]}
                  >
                    <ThemedText type="small" style={{ color: AstroBarColors.warning }}>
                      En Proceso
                    </ThemedText>
                  </Pressable>
                  <Pressable
                    onPress={() => updateTicketStatus(ticket.id, "closed")}
                    style={[tabStyles.actionBtn, { backgroundColor: AstroBarColors.success + "20" }]}
                  >
                ...
                <View style={[tabStyles.statusBadge, { backgroundColor: AstroBarColors.success + "20" }]}>
                  <ThemedText type="small" style={{ color: AstroBarColors.success }}>
                    Cerrado
                  </ThemedText>
                </View>
              ...
              <>
                <ScrollView style={{ flex: 1, padding: Spacing.lg }}>
                  {messages.map((msg) => (
                    <View key={msg.id} style={{ marginBottom: Spacing.md, alignItems: msg.isBot ? "flex-start" : "flex-end" }}>
                      <View style={{ backgroundColor: msg.isBot ? theme.card : AstroBarColors.primary + "20", padding: Spacing.md, borderRadius: 12, maxWidth: "80%" }}>
                ...
                <View style={{ padding: Spacing.lg, borderTopWidth: 1, borderTopColor: theme.border }}>
                  ...
                  <Pressable
                    ...
                    style={{ backgroundColor: AstroBarColors.primary, padding: Spacing.md, borderRadius: 8, alignItems: "center", opacity: sending || !replyText.trim() ? 0.5 : 1 }}
                  >
                    {sending ? (
